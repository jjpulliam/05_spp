...
spp_rcbd_mix_mod <- lmer(yield_kgha ~ krate_kgha*nrate_kgha
                         + (1|rep/krate_kgha),
                         data = spp_rcbd_dfw)
#this is the fixed effect model * takes care of both independent and interaction and the 1 line rep is random? we are nesting krate inside of rep and we go from largest level which is rep to the smallest which is krate
...
#the anova is how we determine if we want to extract the means 
```
